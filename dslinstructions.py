...
class Variable(Value):
    ...
    def SetProgramOrigin(self, po, override):
        if self.programOrigin != None and not override:
            return
    ...
    def GetSsaName(self):
        if self.programOrigin == None:
            return self.name + "." + str(self.ssaIndex)
...
class ArrayCall(Value):
    ...
    def AnalyzeArrayIndexOnlyConst(self, d):
        if not self.name in d:
            d[self.name] = True
        if not isinstance(self.index, Immediate):
            d[self.name] = False
        return
    ...
    def ToString(self):
        retString = ""
        if self.programOrigin != None:
            retString = retString + self.programOrigin + "."
...
class Conditional(Value):
    def __init__(self, cs, tp, fp):
        ...
        self.falsePath = fp
        self.programOrigin = None
        print(cs, tp, fp, self.programOrigin)
...
class Macro(Value):
    ...
    def ToString(self):
        ...
        if self.programOrigin == None:
            return "macro " + self.name + "(" + paramString + ") {\n" + stmtString + "\n}"
...
class Loop(Value):
    ...
    def ToString(self):
        retString = (
            "for (" + self.i.ToString() + " = " + str(self.lb.ToString()) + " to " + str(self.ub.ToString()) + ") {\n"
        )
        for e in self.expr:
...
class Loop2(Value):
    ...
    def ToString(self):
        retString = (
            "for2 (" + self.i.ToString() + " = " + str(self.lb.ToString()) + " to " + str(self.ub.ToString()) + ") {\n"
        )
        for e in self.expr:
            retString = retString + "\t" + e.ToString() + "\n"
        return retString + "}"

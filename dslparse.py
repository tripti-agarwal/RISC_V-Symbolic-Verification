...
constant = pp.Combine(pp.Optional(pp.Literal("-")) + pp.Word(pp.hexnums)) + pp.Optional(
    pp.Combine(pp.Literal(":") + pp.Word(pp.hexnums)), default=":32"
)
tempVar = (
    pp.Optional(pp.Combine(pp.Word(pp.alphas, pp.alphanums) + pp.Literal(".")))
    + pp.Word(pp.alphas, pp.alphanums + "_")
    + pp.Optional(pp.Combine(pp.Literal(":") + pp.Word(pp.hexnums)), default=":32")
)
hexConstant = pp.Combine("0x" + pp.Word(pp.hexnums)) + pp.Optional(
    pp.Combine(pp.Literal(":") + pp.Word(pp.hexnums)), default=":32"
)
...
arrayVar = (tempVar + pp.Suppress(pp.Literal("[")) + orExpression + pp.Suppress(pp.Literal("]"))).setParseAction(
    pushArrayVar
)
...
factor = (
    arrayVar
    | callVar
    | (label | tempVar | hexConstant | constant).setParseAction(CreateVar)
    | (pp.Suppress(pp.Literal("(")) + orExpression + pp.Suppress(pp.Literal(")")))
)
...
ifExpression = (
    orExpression
    + pp.Optional(comparator + orExpression + pp.Literal("?") + orExpression + pp.Literal(":") + orExpression)
).setParseAction(parseIfExpression)
...
function = (
    ...
    + tempVar
    + pp.Suppress(lpar)
    + pp.Optional(paramList)
    + pp.Suppress(rpar)
    + pp.Suppress(pp.Literal("{"))
    + pp.OneOrMore(stmt)
    + pp.Suppress(pp.Literal("}"))
).setParseAction(parseFunction)
...
macro = (
    ...
    + pp.OneOrMore(stmt)
    + pp.Suppress(pp.Literal("}"))
).setParseAction(parseMacro)

forloop = (
    pp.Suppress(pp.Literal("for"))
    + pp.Suppress(pp.Literal("("))
    + factor

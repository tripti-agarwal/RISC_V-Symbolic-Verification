...
class DepGraph:
    ...
    def AddDslInstructionHelper(self, inst, extVars):
        if isinstance(inst, di.Variable):
            return self.CreateVertexVariable(inst, extVars)
        elif isinstance(inst, di.Immediate):
            return self.CreateVertexImmediate(inst, extVars)
        elif isinstance(inst, di.FunctionCall):
            return self.CreateVertexFuncCall(inst, extVars)
        elif isinstance(inst, di.ArrayCall):
            return self.CreateVertexArrayLoad(inst, extVars)
        elif isinstance(inst, di.BinOperation):
            return self.CreateVertexBinOp(inst, extVars)
        elif isinstance(inst, di.Statement):
            if inst.comparator == "<-":
                return self.CreateVertexArrayStore(inst, extVars)
            elif inst.comparator == "=":
                if isinstance(inst.lhs, di.ArrayCall):
                    return self.CreateVertexArrayStore(inst, extVars)
                else:
                    return self.CreateVertexAssign(inst, extVars)
            else:
                return self.CreateVertexCompare(inst, extVars)
        elif isinstance(inst, di.Conditional):
            return self.CreateVertexCond(inst, extVars)
        elif isinstance(inst, di.UnOperation):
            return self.CreateVertexUnOp(inst, extVars)
        elif isinstance(inst, di.DataRegion):
            return self.CreateVertexDataRegion(inst, extVars)
        else:
            sys.exit("%s is not something I had in mind. DepGraph->AddDslInstructionHelper" % (inst))
    ...
    def CreateVertexFuncCall(self, func, extVars):
        ...
        if fVertex.name == "merge":
            ...
            assert (
                fcVertex.operands[2].type == VertexNode.VertexType.IMM
                and fcVertex.operands[3].type == VertexNode.VertexType.IMM
            )
            ...
            sys.exit("Unexpected built-in function name : " + fVertex.name)
    ...
    def CreateVertexBinOp(self, binop, extVars):
        ...
        # print(boVertex.operator)
        # print(boVertex.value)
        # print(boVertex.name)
        # print(boVertex.index)
        # print(boVertex.programOrigin)
        # print(boVertex.type)
        # print(boVertex.bitlength)
        # print("boVertex=", boVertex, " ", boVertex.index)
        return boVertex
    ...
    def CreateVertexArrayStore(self, arrst, extVars):
        ...
        if oaVertex == None:
            oaVertex = self.FindVertexWithNameFromList(
                arrst.lhs.name, arrst.lhs.oldSsaIndex, arrst.lhs.programOrigin, extVars
            )
            if oaVertex != None:
    ...
    def FindVertexWithNameFromList(self, n, i, po, lst):
        for v in lst:
            if v.name == n and v.index == i and v.programOrigin == po:
                return v
        return None
    ...
    def FindArrayWithName(self, n, i, po):
        for a in [v for v in self.vertices if v.type == VertexNode.VertexType.ARR]:
            if a.name == n and a.index == i and a.programOrigin == po:
                return a
        return None
...
class VertexNode:
    class VertexType(Enum):
        ...
        def IsConstant(t):
            return t == VertexNode.VertexType.IMM or t == VertexNode.VertexType.FUNC

        def IsVarOrTemp(t):
            return t == VertexNode.VertexType.VAR or t == VertexNode.VertexType.TEMP
    ...
    class OpCode(Enum):
        ...
        def IsComparison(oc):
            return oc in [
                VertexNode.OpCode.EQ,
                VertexNode.OpCode.NE,
                VertexNode.OpCode.LT,
                VertexNode.OpCode.LE,
                VertexNode.OpCode.GT,
                VertexNode.OpCode.GE,
            ]

        def IsBinaryOp(oc):
            return oc in [
                VertexNode.OpCode.PLUS,
                VertexNode.OpCode.MINUS,
                VertexNode.OpCode.AND,
                VertexNode.OpCode.OR,
                VertexNode.OpCode.XOR,
                VertexNode.OpCode.SHL,
                VertexNode.OpCode.SHR,
                VertexNode.OpCode.ROL,
                VertexNode.OpCode.ROR,
                VertexNode.OpCode.MUL,
                VertexNode.OpCode.DIV,
            ]
        ...
        def GetCompOpCode(s):
            if s == "==":
                return VertexNode.OpCode.EQ
            elif s == "!=":
                return VertexNode.OpCode.NE
            elif s == "<":
                return VertexNode.OpCode.LT
            elif s == "<=":
                return VertexNode.OpCode.LE
            elif s == ">":
                return VertexNode.OpCode.GT
            elif s == ">=":
                return VertexNode.OpCode.GE

        def GetBinOpCode(s):
            if s == "+":
                return VertexNode.OpCode.PLUS
            elif s == "-":
                return VertexNode.OpCode.MINUS
            elif s == "&":
                return VertexNode.OpCode.AND
            elif s == "|":
                return VertexNode.OpCode.OR
            elif s == "^":
                return VertexNode.OpCode.XOR
            elif s == "<<":
                return VertexNode.OpCode.SHL
            elif s == ">>":
                return VertexNode.OpCode.SHR
            elif s == "<<<":
                return VertexNode.OpCode.ROL
            elif s == ">>>":
                return VertexNode.OpCode.ROR
            elif s == "!":
                return VertexNode.OpCode.NOT
            elif s == "*":
                return VertexNode.OpCode.MUL
            elif s == "/":
                return VertexNode.OpCode.DIV
    ...
    def RemoveOperand(self, v):
        ...
        for i in range(0, len(self.operands)):
            if self.operands[i] == None:
                continue
            if self.operands[i] == v:
                self.operands[i] == None
    ...
    def __eq__(self, other):
        ...
        return self.name == other.name and self.index == other.index and self.programOrigin == other.programOrigin
    ...
    def VertexNameToSmt(self):
        assert self.type != VertexNode.VertexType.NONE and self.type != VertexNode.VertexType.FUNC
        ...
        if self.type == VertexNode.VertexType.ARR:
            return z3.Array(
                self.__str__(), z3.BitVecSort(self.arrayIndexBitlength), z3.BitVecSort(self.arrayElBitlength)
            )
    ...
    def VertexSubGraphToSmt(self):
        if self.type == VertexNode.VertexType.VAR:
            ...
            if self.operator == None:
                return self.VertexNameToSmt()
            if self.operator == VertexNode.OpCode.NONE:
                return self.VertexNameToSmt()

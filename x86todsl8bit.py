...
def SetupOperand64Bit(x):
    ...
    if (isImmediate(x[1]) or Is64Register(x[1])) and Is64Register(x[2]):
        ...
        source = "tempSource64:64"
        dest = x[2] + ":64"
        addr = CalculateAddress(x[1])
        tempString = LoadFromMemoryString(addr, 8, source)
    else:
        sys.exit("Convertsubl operand error: %s" % (x))
...
def SetupOperandForShifts64Bit(x):
    ...
    if isImmediate(x[1]) and Is64Register(x[2]):
        ...
        sys.exit("Convertshrq operand error: %s" % (x))
...
def SetupOperandForShifts32Bit(x):
    ...
    if isImmediate(x[1]) and Is32Register(x[2]):
        ...
        sys.exit("ConvertRorl operand error: %s" % (x))
...
def ConvertDecl(x):
    ...
    if Is32Register(x[1]):
        ...
        addr = CalculateAddress(x[1])
        tempString = (
            LoadFromMemoryString(addr, 4, dest)
            + oldDest
            + " = "
            + dest
            + ";\n"
...
def ConvertPopq(x):
    if Is64Register(x[1]):
        addr = "rsp:64"
        dest = x[1] + ":64"
        tempString = LoadFromMemoryString(addr, 8, dest) + addr + " = " + addr + " + 8:64" + ";\n"
    else:
...
def ConvertShrq(x):
    ...
    tempString = tempString + "cf:1 = split(" + dest + ", 0, 0);\n"
    if int(x[1], 0) == 1:
        tempString = tempString + "of:1 = split(" + oldDest + ", 63, 63);\n"
    else:
        tempString = tempString + "of:1 = builtin_undef;\n"
...
def ConvertMovd(x):
    ...
    # Movd 32-bit mem, xmm
        ...
        dest = [x[2] + "_0", x[2] + "_1", x[2] + "_2", x[2] + "_3"]
        tempString = (
            LoadFromMemoryString(addr, 4, dest[0]) + dest[1] + " = 0;\n" + dest[2] + " = 0;\n" + dest[3] + " = 0;\n"
        )
    else:
...
def ConvertMovq(x):
    ...
    # 64-bit mem to 64-bit reg
        ...
        dest = x[2] + ":64"
        dest32 = Get32BitRepFromRegister(x[2])[0]
        tempString = LoadFromMemoryString(addr, 8, dest) + dest32 + " = split(" + dest + ", 0, 31);\n"

...
parser.add_argument(
    "--mem_model",
    type=int,
    help="Memory model can be based on 8-bit value, or a 32-bit value (assuming there will be 4-byte aligned memory operations. Options: 8, 32) Defaults to 32",
)
parser.add_argument(
    "--verif_mode",
...
parser.add_argument(
    "--max_unknown_count",
    type=int,
    help="Number of Unknown results to encounter before exiting verification. Defaults to 1",
)
...
ConvertToSSAForms(preAst, p1Ast, p2Ast, postAst)  # -----focus
...
for d in programGraph.vertices:
    print(d.name, d.index, d.operator, d.programOrigin)
####################################################################################################
